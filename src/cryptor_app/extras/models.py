...
def insertCookie(cookie_id, cookie_owner_id, cookie_owner_username, ts, cookie_expire_time, cookie_owner_ts, cookie_owner_last_updated, cookie_expired = False):
  ...
  cur.execute(
    ...
    ''', 
    {
      ...
      'cookie_owner_ts':cookie_owner_ts, 
      'cookie_owner_last_updated':cookie_owner_last_updated, 
...
def verifyCookie():
  ...
  try:
    ...
    cur = con.cursor()
    cur.execute(
      '''SELECT * FROM cookies WHERE cookie_expired = :cookie_expired''', 
...
def logout_func(cookie_id):
  ...
  try:
    with con:
      ...
      cur.execute(
        '''
          UPDATE cookies SET cookie_expired = :cookie_expired WHERE cookie_id = :cookie_id
        ''', 
...
def renew_cookie(cookie_id, cookie_expire_time):
  ...
  try:
    with con:
      ...
      cur.execute(
        '''
          UPDATE cookies SET cookie_expire_time = :cookie_expire_time WHERE cookie_id = :cookie_id
        ''', 
...
def insertFile(file_id, owner_name, data_file, cipher_aes, tag, session_key, ts, file_title, file_for):
  ...
  try:
    ...
    with con:
      ...
      cur.execute(
        '''
          INSERT INTO lockedfiles VALUES(
            :file_id, 
            :owner_name, 
            :data_file, 
            :cipher_aes, 
...
def generate_keys():
  ...
  try:
    ...
    cur = con.cursor()
    cur.execute(
      '''INSERT INTO keys VALUES(:key_id, :key_data, :session_key)''', 
      {"key_data": public_bytes, "key_id": pbl, "session_key":session_key}
    )
    cur.execute(
      '''INSERT INTO keys VALUES(:key_id, :key_data, :session_key)''', 
      {"key_data": private_bytes, "key_id": pvt, "session_key":enc_session_key}
    )
    con.commit()
    con.close()
